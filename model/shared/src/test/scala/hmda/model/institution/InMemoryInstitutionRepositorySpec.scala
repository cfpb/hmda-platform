...
class InMemoryInstitutionRepositorySpec extends WordSpec with MustMatchers {
  ...
  val institutionRepository = new InMemoryInstitutionRepository(Set(
    Institution("1", "Test Bank 1", Set(ExternalId("99-1234567", FederalTaxId), ExternalId("123456", RssdId)), CFPB, Bank, hasParent = true, status = Active),
    Institution("2", "Test Bank 2", Set(ExternalId("98-1234567", FederalTaxId), ExternalId("9898989", FdicCertNo)), FDIC, Bank, hasParent = true, status = Inactive),
    Institution("3", "Test Bank 3", Set(ExternalId("97-1234567", FederalTaxId), ExternalId("64646464", OccCharterId)), OCC, SavingsAndLoan, hasParent = false, status = Active)
  ))
