...
trait PublisherComponent2024 extends PGTableNameLoader {
  ...
  sealed trait Year2024Period
  object Year2024Period {
    case object Whole extends Year2024Period
    case object Q1    extends Year2024Period
    case object Q2    extends Year2024Period
    case object Q3    extends Year2024Period
  }
  ...
  class InstitutionsTable(tag: Tag) extends InstitutionsTableBase[InstitutionEntity](tag, panel2024TableName) {
    override def * =
      (
        lei,
        activityYear,
  ...
  val institutionsTable2024 = TableQuery[InstitutionsTable]


  class InstitutionRepository2024(val config: DatabaseConfig[JdbcProfile]) extends TableRepository[InstitutionsTable, String] {

    override val table: config.profile.api.TableQuery[InstitutionsTable] =
      institutionsTable2024
  ...
  def institutionTableQuery2024(p: Year2024Period) = {
    TableQuery(tag => new InstitutionsTable(tag))
  }

  def transmittalSheetTableQuery2024(p: Year2024Period): TableQuery[TransmittalSheetTable] = {
    val tableName = p match {
      case Year2024Period.Whole => ts2024TableName
      case Year2024Period.Q1    => ts2024Q1TableName
      case Year2024Period.Q2    => ts2024Q2TableName
      case Year2024Period.Q3    => ts2024Q3TableName
    }
  ...
  abstract class LarTableBase[T](tag: Tag, tableName: String) extends Table[T](tag, tableName) {
    ...
    def larEntityImpl2024Projection =
      (
      ...
      ) <> ((LarEntityImpl2024.apply _).tupled, LarEntityImpl2024.unapply)
    ...
    def larPartOneProjection =
      (
        ...
        county,
        tract
      ) <> ((LarPartOne2024.apply _).tupled, LarPartOne2024.unapply)
    ...
    def larPartTwoProjection =
      (
        ...
        raceApplicant4,
        raceApplicant5
      ) <> ((LarPartTwo2024.apply _).tupled, LarPartTwo2024.unapply)
    ...
    def larPartThreeProjection =
      (
        ...
        ageCoApplicant,
        income
      ) <> ((LarPartThree2024.apply _).tupled, LarPartThree2024.unapply)
    ...
    def larPartFourProjection =
      (
        ...
        totalPoints,
        originationCharges
      ) <> ((LarPartFour2024.apply _).tupled, LarPartFour2024.unapply)
    ...
    def larPartFiveProjection =
      ...
      ) <> ((LarPartFive2024.apply _).tupled, LarPartFive2024.unapply)
    ...
    def larPartSixProjection =
      ...
      ) <> ((LarPartSix2024.apply _).tupled, LarPartSix2024.unapply)
    ...
    def larPartSevenProjection =
      ...
      ) <> ((LarPartSeven2024.apply _).tupled, LarPartSeven2024.unapply)

  }

  def createTransmittalSheetRepository2024(config: DatabaseConfig[JdbcProfile], p: Year2024Period) =
    new TsRepository[TransmittalSheetTable](config, transmittalSheetTableQuery2024(p))


  class RealLarTable2024(tag: Tag, tableName: String) extends LarTableBase[LarEntityImpl2024](tag, tableName) {
    def * = larEntityImpl2024Projection
  }

  class QALarTableBase2024(tag: Tag, tableName: String)
    extends LarTableBase[QAEntity[LarEntityImpl2024]](tag, tableName)
      with QATableBase[LarEntityImpl2024] {
    def * = (larEntityImpl2024Projection, fileName,timeStamp) <> ((QAEntity.apply[LarEntityImpl2024] _).tupled, QAEntity.unapply[LarEntityImpl2024] _)
  }

  def larTableQuery2024(p: Year2024Period) = {
    val tableName = p match {
      case Year2024Period.Whole => lar2024TableName
      case Year2024Period.Q1    => lar2024Q1TableName
      case Year2024Period.Q2    => lar2024Q2TableName
      case Year2024Period.Q3    => lar2024Q3TableName
    }
    TableQuery(tag => new RealLarTable2024(tag, tableName))
  }



  class LarRepository2024Base[LarTable <: RealLarTable2024](val config: DatabaseConfig[JdbcProfile], val table: TableQuery[LarTable])
    ...
    def insert(ts: LarEntityImpl2024): Future[Int] =
      db.run(table += ts)

    def findByLei(lei: String): Future[Seq[LarEntityImpl2024]] =
      db.run(table.filter(_.lei === lei).result)
    ...
    def getAllLARs(bankIgnoreList: Array[String]): DatabasePublisher[LarEntityImpl2024] =
      db.stream(
    ...
    protected def getAllLARsQuery(bankIgnoreList: Array[String]): Query[LarTable, LarEntityImpl2024, Seq] =
      table.filterNot(_.lei.toUpperCase inSet bankIgnoreList)
  }

  def createLarRepository2024(config: DatabaseConfig[JdbcProfile], p: Year2024Period) =
    new LarRepository2024Base(config, larTableQuery2024(p))

  def validationLarData2024(p: Year2024Period): LarData = LarData[LarEntityImpl2024, RealLarTable2024](larTableQuery2024(p))(_.lei)

  def validationTSData2024(p: Year2024Period): TsData =
    TsData[TransmittalSheetEntity, TransmittalSheetTable](transmittalSheetTableQuery2024(p))(_.lei, _.totalLines, _.submissionId)
  def validationPanelData2024(p: Year2024Period): PanelData =
    PanelData[InstitutionEntity, InstitutionsTable](institutionTableQuery2024(p))(_.lei, _.hmdaFiler)
  ...
  class ModifiedLarTable(tag: Tag) extends ModifiedLarTableBase[ModifiedLarEntityImpl](tag, mlar2024TableName) {
    override def * = modifiedLarEntityImplProjection
  }

  val mlarTable2024 = TableQuery[ModifiedLarTable]

  class ModifiedLarRepository2024(val config: DatabaseConfig[JdbcProfile]) extends TableRepository[ModifiedLarTable, String] {
  ...
  val validationMLarData2024: LarData =
    LarData[ModifiedLarEntityImpl, ModifiedLarTable](mlarTable2024)(_.lei)
