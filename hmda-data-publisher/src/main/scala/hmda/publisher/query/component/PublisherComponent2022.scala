...
trait PublisherComponent2022 extends PGTableNameLoader {
  ...
  def createTransmittalSheetRepository2022(config: DatabaseConfig[JdbcProfile], p: Year2022Period) =
    new TsRepository[TransmittalSheetTable](config, transmittalSheetTableQuery2022(p))
  ...
  class ModifiedLarTable(tag: Tag) extends ModifiedLarTableBase[ModifiedLarEntityImpl](tag, mlar2022TableName) {
    override def * = modifiedLarEntityImplProjection
  }

  val mlarTable2022 = TableQuery[ModifiedLarTable]

  class ModifiedLarRepository2022(val config: DatabaseConfig[JdbcProfile]) extends TableRepository[ModifiedLarTable, String] {
  ...
  val validationMLarData2022: LarData =
    LarData[ModifiedLarEntityImpl, ModifiedLarTable](mlarTable2022)(_.lei)
