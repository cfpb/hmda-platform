...
import hmda.publisher.query.lar.{ LarEntityImpl2025, _ }
...
// $COVERAGE-OFF$
trait PublisherComponent2025 extends PGTableNameLoader {

  import dbConfig.profile.api._

  sealed trait Year2025Period
  object Year2025Period {
    case object Whole extends Year2025Period
    case object Q1    extends Year2025Period
    case object Q2    extends Year2025Period
    case object Q3    extends Year2025Period
  }
  ...
  class InstitutionsTable(tag: Tag) extends InstitutionsTableBase[InstitutionEntity](tag, panel2025TableName) {
    override def * =
      (
        lei,
        activityYear,
  ...
  val institutionsTable2025 = TableQuery[InstitutionsTable]


  class InstitutionRepository2025(val config: DatabaseConfig[JdbcProfile]) extends TableRepository[InstitutionsTable, String] {

    override val table: config.profile.api.TableQuery[InstitutionsTable] =
      institutionsTable2025
  ...
  def institutionTableQuery2025(p: Year2025Period) = {
    TableQuery(tag => new InstitutionsTable(tag))
  }

  def transmittalSheetTableQuery2025(p: Year2025Period): TableQuery[TransmittalSheetTable] = {
    val tableName = p match {
      case Year2025Period.Whole => ts2025TableName
      case Year2025Period.Q1    => ts2025Q1TableName
      case Year2025Period.Q2    => ts2025Q2TableName
      case Year2025Period.Q3    => ts2025Q3TableName
    }
  ...
  abstract class LarTableBase[T](tag: Tag, tableName: String) extends Table[T](tag, tableName) {
    ...
    def larEntityImpl2025Projection =
      (
      ...
      ) <> ((LarEntityImpl2025.apply _).tupled, LarEntityImpl2025.unapply)
    ...
    def larPartOneProjection =
      (
        ...
        county,
        tract
      ) <> ((LarPartOne2025.apply _).tupled, LarPartOne2025.unapply)
    ...
    def larPartTwoProjection =
      (
        ...
        raceApplicant4,
        raceApplicant5
      ) <> ((LarPartTwo2025.apply _).tupled, LarPartTwo2025.unapply)
    ...
    def larPartThreeProjection =
      (
        ...
        ageCoApplicant,
        income
      ) <> ((LarPartThree2025.apply _).tupled, LarPartThree2025.unapply)
    ...
    def larPartFourProjection =
      (
        ...
        totalPoints,
        originationCharges
      ) <> ((LarPartFour2025.apply _).tupled, LarPartFour2025.unapply)
    ...
    def larPartFiveProjection =
      ...
      ) <> ((LarPartFive2025.apply _).tupled, LarPartFive2025.unapply)
    ...
    def larPartSixProjection =
      ...
      ) <> ((LarPartSix2025.apply _).tupled, LarPartSix2025.unapply)
    ...
    def larPartSevenProjection =
      ...
      ) <> ((LarPartSeven2025.apply _).tupled, LarPartSeven2025.unapply)

  }

  def createTransmittalSheetRepository2025(config: DatabaseConfig[JdbcProfile], p: Year2025Period) =
    new TsRepository[TransmittalSheetTable](config, transmittalSheetTableQuery2025(p))


  class RealLarTable2025(tag: Tag, tableName: String) extends LarTableBase[LarEntityImpl2025](tag, tableName) {
    def * = larEntityImpl2025Projection
  }

  class QALarTableBase2025(tag: Tag, tableName: String)
    extends LarTableBase[QAEntity[LarEntityImpl2025]](tag, tableName)
      with QATableBase[LarEntityImpl2025] {
    def * = (larEntityImpl2025Projection, fileName,timeStamp) <> ((QAEntity.apply[LarEntityImpl2025] _).tupled, QAEntity.unapply[LarEntityImpl2025] _)
  }

  def larTableQuery2025(p: Year2025Period) = {
    val tableName = p match {
      case Year2025Period.Whole => lar2025TableName
      case Year2025Period.Q1    => lar2025Q1TableName
      case Year2025Period.Q2    => lar2025Q2TableName
      case Year2025Period.Q3    => lar2025Q3TableName
    }
    TableQuery(tag => new RealLarTable2025(tag, tableName))
  }



  class LarRepository2025Base[LarTable <: RealLarTable2025](val config: DatabaseConfig[JdbcProfile], val table: TableQuery[LarTable])
    ...
    def insert(ts: LarEntityImpl2025): Future[Int] =
      db.run(table += ts)

    def findByLei(lei: String): Future[Seq[LarEntityImpl2025]] =
      db.run(table.filter(_.lei === lei).result)
    ...
    def getAllLARs(bankIgnoreList: Array[String]): DatabasePublisher[LarEntityImpl2025] =
      db.stream(
    ...
    protected def getAllLARsQuery(bankIgnoreList: Array[String]): Query[LarTable, LarEntityImpl2025, Seq] =
      table.filterNot(_.lei.toUpperCase inSet bankIgnoreList)
  }

  def createLarRepository2025(config: DatabaseConfig[JdbcProfile], p: Year2025Period) =
    new LarRepository2025Base(config, larTableQuery2025(p))

  def validationLarData2025(p: Year2025Period): LarData = LarData[LarEntityImpl2025, RealLarTable2025](larTableQuery2025(p))(_.lei)

  def validationTSData2025(p: Year2025Period): TsData =
    TsData[TransmittalSheetEntity, TransmittalSheetTable](transmittalSheetTableQuery2025(p))(_.lei, _.totalLines, _.submissionId)
  def validationPanelData2025(p: Year2025Period): PanelData =
    PanelData[InstitutionEntity, InstitutionsTable](institutionTableQuery2025(p))(_.lei, _.hmdaFiler)
  ...
  class ModifiedLarTable(tag: Tag) extends ModifiedLarTableBase[ModifiedLarEntityImpl](tag, mlar2025TableName) {
    override def * = modifiedLarEntityImplProjection
  }

  val mlarTable2025 = TableQuery[ModifiedLarTable]

  class ModifiedLarRepository2025(val config: DatabaseConfig[JdbcProfile]) extends TableRepository[ModifiedLarTable, String] {
  ...
  val validationMLarData2025: LarData =
    LarData[ModifiedLarEntityImpl, ModifiedLarTable](mlarTable2025)(_.lei)
