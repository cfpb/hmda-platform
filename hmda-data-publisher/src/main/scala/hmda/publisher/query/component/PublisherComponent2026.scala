...
// $COVERAGE-OFF$
trait PublisherComponent2026 extends PGTableNameLoader {

  import dbConfig.profile.api._

  sealed trait Year2026Period
  object Year2026Period {
    case object Whole extends Year2026Period
    case object Q1    extends Year2026Period
    case object Q2    extends Year2026Period
    case object Q3    extends Year2026Period
  }
  ...
  class InstitutionsTable(tag: Tag) extends InstitutionsTableBase[InstitutionEntity](tag, panel2026TableName) {
    override def * =
      (
        lei,
        activityYear,
  ...
  val institutionsTable2026 = TableQuery[InstitutionsTable]


  class InstitutionRepository2026(val config: DatabaseConfig[JdbcProfile]) extends TableRepository[InstitutionsTable, String] {

    override val table: config.profile.api.TableQuery[InstitutionsTable] =
      institutionsTable2026
  ...
  def institutionTableQuery2026(p: Year2026Period) = {
    TableQuery(tag => new InstitutionsTable(tag))
  }

  def transmittalSheetTableQuery2026(p: Year2026Period): TableQuery[TransmittalSheetTable] = {
    val tableName = p match {
      case Year2026Period.Whole => ts2026TableName
      case Year2026Period.Q1    => ts2026Q1TableName
      case Year2026Period.Q2    => ts2026Q2TableName
      case Year2026Period.Q3    => ts2026Q3TableName
    }
  ...
  abstract class LarTableBase[T](tag: Tag, tableName: String) extends Table[T](tag, tableName) {
    ...
    def larEntityImpl2026Projection =
      (
      ...
      ) <> ((LarEntityImpl2026.apply _).tupled, LarEntityImpl2026.unapply)
    ...
    def larPartOneProjection =
      (
        ...
        county,
        tract
      ) <> ((LarPartOne2026.apply _).tupled, LarPartOne2026.unapply)
    ...
    def larPartTwoProjection =
      (
        ...
        raceApplicant4,
        raceApplicant5
      ) <> ((LarPartTwo2026.apply _).tupled, LarPartTwo2026.unapply)
    ...
    def larPartThreeProjection =
      (
        ...
        ageCoApplicant,
        income
      ) <> ((LarPartThree2026.apply _).tupled, LarPartThree2026.unapply)
    ...
    def larPartFourProjection =
      (
        ...
        totalPoints,
        originationCharges
      ) <> ((LarPartFour2026.apply _).tupled, LarPartFour2026.unapply)
    ...
    def larPartFiveProjection =
      ...
      ) <> ((LarPartFive2026.apply _).tupled, LarPartFive2026.unapply)
    ...
    def larPartSixProjection =
      ...
      ) <> ((LarPartSix2026.apply _).tupled, LarPartSix2026.unapply)
    ...
    def larPartSevenProjection =
      ...
      ) <> ((LarPartSeven2026.apply _).tupled, LarPartSeven2026.unapply)

  }

  def createTransmittalSheetRepository2026(config: DatabaseConfig[JdbcProfile], p: Year2026Period) =
    new TsRepository[TransmittalSheetTable](config, transmittalSheetTableQuery2026(p))


  class RealLarTable2026(tag: Tag, tableName: String) extends LarTableBase[LarEntityImpl2026](tag, tableName) {
    def * = larEntityImpl2026Projection
  }

  class QALarTableBase2026(tag: Tag, tableName: String)
    extends LarTableBase[QAEntity[LarEntityImpl2026]](tag, tableName)
      with QATableBase[LarEntityImpl2026] {
    def * = (larEntityImpl2026Projection, fileName,timeStamp) <> ((QAEntity.apply[LarEntityImpl2026] _).tupled, QAEntity.unapply[LarEntityImpl2026] _)
  }

  def larTableQuery2026(p: Year2026Period) = {
    val tableName = p match {
      case Year2026Period.Whole => lar2026TableName
      case Year2026Period.Q1    => lar2026Q1TableName
      case Year2026Period.Q2    => lar2026Q2TableName
      case Year2026Period.Q3    => lar2026Q3TableName
    }
    TableQuery(tag => new RealLarTable2026(tag, tableName))
  }



  class LarRepository2026Base[LarTable <: RealLarTable2026](val config: DatabaseConfig[JdbcProfile], val table: TableQuery[LarTable])
    ...
    def insert(ts: LarEntityImpl2026): Future[Int] =
      db.run(table += ts)

    def findByLei(lei: String): Future[Seq[LarEntityImpl2026]] =
      db.run(table.filter(_.lei === lei).result)
    ...
    def getAllLARs(bankIgnoreList: Array[String]): DatabasePublisher[LarEntityImpl2026] =
      db.stream(
    ...
    protected def getAllLARsQuery(bankIgnoreList: Array[String]): Query[LarTable, LarEntityImpl2026, Seq] =
      table.filterNot(_.lei.toUpperCase inSet bankIgnoreList)
  }

  def createLarRepository2026(config: DatabaseConfig[JdbcProfile], p: Year2026Period) =
    new LarRepository2026Base(config, larTableQuery2026(p))

  def validationLarData2026(p: Year2026Period): LarData = LarData[LarEntityImpl2026, RealLarTable2026](larTableQuery2026(p))(_.lei)

  def validationTSData2026(p: Year2026Period): TsData =
    TsData[TransmittalSheetEntity, TransmittalSheetTable](transmittalSheetTableQuery2026(p))(_.lei, _.totalLines, _.submissionId)
  def validationPanelData2026(p: Year2026Period): PanelData =
    PanelData[InstitutionEntity, InstitutionsTable](institutionTableQuery2026(p))(_.lei, _.hmdaFiler)
  ...
  class ModifiedLarTable(tag: Tag) extends ModifiedLarTableBase[ModifiedLarEntityImpl](tag, mlar2026TableName) {
    override def * = modifiedLarEntityImplProjection
  }

  val mlarTable2026 = TableQuery[ModifiedLarTable]

  class ModifiedLarRepository2026(val config: DatabaseConfig[JdbcProfile]) extends TableRepository[ModifiedLarTable, String] {
  ...
  val validationMLarData2026: LarData =
    LarData[ModifiedLarEntityImpl, ModifiedLarTable](mlarTable2026)(_.lei)
