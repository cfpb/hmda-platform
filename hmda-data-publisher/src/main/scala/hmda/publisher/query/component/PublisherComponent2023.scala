...
trait PublisherComponent2023 extends PGTableNameLoader {
  ...
  def createTransmittalSheetRepository2023(config: DatabaseConfig[JdbcProfile], p: Year2023Period) =
    new TsRepository[TransmittalSheetTable](config, transmittalSheetTableQuery2023(p))
  ...
  class QALarTableBase2023(tag: Tag, tableName: String)
    extends LarTableBase[QAEntity[LarEntityImpl2023]](tag, tableName)
      with QATableBase[LarEntityImpl2023] {
    def * = (larEntityImpl2023Projection, fileName,timeStamp) <> ((QAEntity.apply[LarEntityImpl2023] _).tupled, QAEntity.unapply[LarEntityImpl2023] _)
  }
  ...
  class ModifiedLarTable(tag: Tag) extends ModifiedLarTableBase[ModifiedLarEntityImpl](tag, mlar2023TableName) {
    override def * = modifiedLarEntityImplProjection
  }

  val mlarTable2023 = TableQuery[ModifiedLarTable]

  class ModifiedLarRepository2023(val config: DatabaseConfig[JdbcProfile]) extends TableRepository[ModifiedLarTable, String] {
  ...
  val validationMLarData2023: LarData =
    LarData[ModifiedLarEntityImpl, ModifiedLarTable](mlarTable2023)(_.lei)
