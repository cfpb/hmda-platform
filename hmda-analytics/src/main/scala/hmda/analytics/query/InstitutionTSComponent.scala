...
import hmda.query.ts.{TransmittalSheetEntity, TransmittalSheetRepository}
import slick.basic.DatabaseConfig
import slick.jdbc.JdbcProfile

import scala.concurrent.Future

trait InstitutionTSComponent {

  import dbConfig.profile.api._

  class InstitutionsTable(tag: Tag, tableName: String) extends Table[InstitutionTSEntity](tag, tableName) {
    ...
    def * =
      (
        ...
        notes
      ) <> (InstitutionTSEntity.tupled, InstitutionTSEntity.unapply)
  }
  ...
  class InstitutionRepository(val config: DatabaseConfig[JdbcProfile], tableName: String)
    ...
    def updateByLei(inst: InstitutionTSEntity): Future[Int] =
      db.run(table.insertOrUpdate(inst))
