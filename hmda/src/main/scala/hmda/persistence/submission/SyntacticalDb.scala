...
import hmda.persistence.submission.repository.AggregationCountRepository

import scala.collection.JavaConverters._

class SyntacticalDb private (override val connector: CassandraConnection) extends Database[SyntacticalDb](connector) {
  object aggregationCountRepository extends AggregationCountRepository with connector.Connector
}
