...
object Q611 extends EditCheck[LoanApplicationRegister] {
  ...
  override def apply(lar: LoanApplicationRegister): ValidationResult = {
    ...
    when(
      lar.action.actionTakenType is equalTo(LoanOriginated) and (lar.lienStatus is equalTo(
        SecuredBySubordinateLien)) and (rs is greaterThan(8.5))) {
