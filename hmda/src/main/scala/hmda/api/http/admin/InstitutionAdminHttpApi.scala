...
import hmda.messages.institution.InstitutionCommands.{ CreateInstitution, DeleteInstitution, GetInstitution, ModifyInstitution }
        ...
        entity(as[Institution]) { institution =>
          val institutionPersistence = {
            if (institution.activityYear == 2018) {
              sharding.entityRefFor(InstitutionPersistence.typeKey, s"${InstitutionPersistence.name}-${institution.LEI}")
            } else {
          ...
          timedPost { uri =>
            ...
            timedPut { uri =>
              val originalInst: Future[Option[Institution]] = institutionPersistence ? (
                ref => GetInstitution(ref)
              )

              def modifyCall(originalInstOpt: Option[Institution]): Future[InstitutionEvent] = {
              ...
              onComplete(fModified) {
                ...
                case Success(_) =>
                  complete(ToResponseMarshallable(HttpResponse(StatusCodes.BadRequest)))
                case Failure(error) =>
                  failedResponse(StatusCodes.InternalServerError, uri, error)
              }
            } ~
            timedDelete { uri =>
              ...
              onComplete(fDeleted) {
                case Success(InstitutionDeleted(lei, year)) =>
                  complete(ToResponseMarshallable(StatusCodes.Accepted -> InstitutionDeletedResponse(lei)))
                case Success(InstitutionNotExists(lei)) =>
                  complete(ToResponseMarshallable(StatusCodes.NotFound -> lei))
                case Success(_) =>
                  complete(ToResponseMarshallable(HttpResponse(StatusCodes.BadRequest)))
