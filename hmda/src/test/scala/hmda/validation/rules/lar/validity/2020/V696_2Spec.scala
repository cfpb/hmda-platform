...
import hmda.model.filing.lar.enums.{ ApproveEligible, EmptyAUSResultValue, InvalidAutomatedUnderwritingResultCode }
...
class V696_2Spec extends LarEditCheckSpec {
  ...
  property("AUS Result fields must be valid") {
    forAll(larGen) { lar =>
      ...
      val invalidLar1 = validLar.copy(ausResult = validLar.ausResult.copy(ausResult1 = EmptyAUSResultValue))
      invalidLar1.mustFail

      val invalidLar2 = validLar.copy(ausResult = validLar.ausResult.copy(ausResult3 = new InvalidAutomatedUnderwritingResultCode))
