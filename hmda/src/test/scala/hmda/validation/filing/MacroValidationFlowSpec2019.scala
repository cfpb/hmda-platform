...
import MacroValidationFlow2019._
...
class MacroValidationFlow2019Spec
  ...
  "Macro Validation" must {
    ...
    "fail Q646" in {
      val q646Source = source.map(lar =>
        lar.copy(applicationSubmission = ApplicationSubmissionExempt))
      macroEditAny(q646Source, q646Name, exemptionTaken)
        .map(e => e mustBe MacroValidationError(q646Name))
    }
    ...
    "collect macro edits" in {
      ...
      macroValidation2019(failSource).map(
        xs =>
          xs mustBe List(
