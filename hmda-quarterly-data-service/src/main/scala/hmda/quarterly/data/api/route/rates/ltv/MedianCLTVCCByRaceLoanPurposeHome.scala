...
object MedianCLTVCCByRaceLoanPurposeHome extends RatesGraph(
  "ltv",
  "ltv-cc-re-loan-purpose-home",
  CC_BY_RACE_TITLE,
  CC_BY_RACE_SUBTITLE,
  Category.BY_RACE) {

  override protected def getSummaryByRace(title: String, race: String): CancelableFuture[GraphSeriesSummary] =
    QuarterlyGraphRepo.fetchMedianCLTVByTypeByRaceLoanPurposeHome(Conventional, race, heloc = false, conforming = true)
