...
object MedianInterestRatesCCByRaceLoanPurposeRefinance extends InterestRatesGraph(
  "interest",
  "interest-rates-cc-re-loan-purpose-refinance",
  CC_BY_RACE_TITLE,
  CC_BY_RACE_SUBTITLE,
  Category.BY_RACE) {

  override protected def getSummaryByRace(title: String, race: String): CancelableFuture[GraphSeriesSummary] =
    QuarterlyGraphRepo.fetchMedianInterestRatesByTypeByRaceLoanPurposeRefinance(Conventional, race, heloc = false, conforming = true)
