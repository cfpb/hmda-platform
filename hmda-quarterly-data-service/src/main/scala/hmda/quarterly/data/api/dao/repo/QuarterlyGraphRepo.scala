...
object QuarterlyGraphRepo {
  ...
  def fetchApplicationsVolumeByType(loanType: LoanTypeEnum, heloc: Boolean, conforming: Boolean): Task[Seq[DataPoint]] = {
    val stmts = getAppVolStmts(APP_VOL_MV, APP_VOL_PERIODS, loanType, heloc, conforming) ++ Seq(ordering)
    runQuery(combineStatements(stmts:_*).as[DataPoint])
  }

  def fetchLoansVolumeByType(loanType: LoanTypeEnum, heloc: Boolean, conforming: Boolean): Task[Seq[DataPoint]] = {
    val stmts = getAppVolStmts(APP_VOL_MV, APP_VOL_PERIODS, loanType, heloc, conforming, "and action_taken_type = 1") ++ Seq(ordering)
    runQuery(combineStatements(stmts:_*).as[DataPoint])
  }

  private def getAppVolStmts(mv: String, periods: Seq[String], loanType: LoanTypeEnum, heloc: Boolean, conforming: Boolean, additionalCriteria: String = ""): Seq[SQLActionBuilder] = {
  ...
  private def runQuery[T](query: SqlStreamingAction[Vector[T], T, Effect]): Task[Vector[T]] =
