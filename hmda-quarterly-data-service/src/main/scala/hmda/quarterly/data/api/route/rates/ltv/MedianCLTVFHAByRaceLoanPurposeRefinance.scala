...
object MedianCLTVFHAByRaceLoanPurposeRefinance extends RatesGraph(
  "ltv",
  "ltv-fha-re-loan-purpose-refinance",
  FHA_BY_RACE_TITLE,
  FHA_BY_RACE_SUBTITLE,
  Category.BY_RACE) {

  override protected def getSummaryByRace(title: String, race: String): CancelableFuture[GraphSeriesSummary] =
    QuarterlyGraphRepo.fetchMedianCLTVByTypeByRaceLoanPurposeRefinance(FHAInsured, race, heloc = false, conforming = false)
