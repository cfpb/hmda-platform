...
object MedianInterestRatesFHAByRaceLoanPurposeRefinance extends InterestRatesGraph(
  "interest",
  "interest-rates-fha-re-loan-purpose-refinance",
  FHA_BY_RACE_TITLE,
  FHA_BY_RACE_SUBTITLE,
  Category.BY_RACE) {

  override protected def getSummaryByRace(title: String, race: String): CancelableFuture[GraphSeriesSummary] =
    QuarterlyGraphRepo.fetchMedianInterestRatesByTypeByRaceLoanPurposeRefinance(FHAInsured, race, heloc = false, conforming = false)
