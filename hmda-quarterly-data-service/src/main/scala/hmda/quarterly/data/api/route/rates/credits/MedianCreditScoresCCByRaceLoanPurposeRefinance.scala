...
object MedianCreditScoresCCByRaceLoanPurposeRefinance extends CountRatesGraph(
  "credit",
  "credit-scores-cc-re-loan-purpose-refinance",
  CC_BY_RACE_TITLE,
  CC_BY_RACE_SUBTITLE,
  Category.BY_RACE) {

  override protected def getSummaryByRace(title: String, race: String): CancelableFuture[GraphSeriesSummary] =
    QuarterlyGraphRepo.fetchMedianCreditScoreByTypeByRaceLoanPurposeRefinance(Conventional, race, conforming = true)
