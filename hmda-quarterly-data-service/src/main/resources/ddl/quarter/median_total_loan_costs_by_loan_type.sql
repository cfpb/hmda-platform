DROP MATERIALIZED VIEW  IF EXISTS median_total_loan_costs_by_loan_type_{mv_suffix};

create materialized view median_total_loan_costs_by_loan_type_{mv_suffix} as
  select 
    now() as last_updated,
    case 
    ...
    percentile_cont(0.5) within group(order by total_loan_costs::numeric) 
    as median_total_loan_costs,
  ...
  where action_taken_type = 1
    ...
    and line_of_credits = 2 -- removed heloc, line_of_credits  = 1
