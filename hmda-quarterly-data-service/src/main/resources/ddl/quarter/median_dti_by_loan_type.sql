DROP MATERIALIZED VIEW  IF EXISTS median_dti_by_loan_type_{mv_suffix};

create materialized view median_dti_by_loan_type_{mv_suffix} as
  select 
    now() as last_updated,
    case 
    ...
    percentile_cont(0.5) within group(order by debt_to_incode::decimal) 
    as median_dti,
  ...
  where action_taken_type = 1
    ...
    and (debt_to_incode::decimal > 0 and debt_to_incode::decimal <= 100) -- threshold
