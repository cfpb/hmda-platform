DROP MATERIALIZED VIEW  IF EXISTS denial_rates_by_race_{mv_suffix};

create materialized view denial_rates_by_race_{mv_suffix} as
  select 
    now() as last_updated,
    case 
    ...
    case
      when count(*) filter(where action_taken_type in (1,2,3)) <= 0 -- ensure denominator != 0
      then 0 
