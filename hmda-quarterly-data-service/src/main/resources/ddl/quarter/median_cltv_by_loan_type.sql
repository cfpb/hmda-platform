DROP MATERIALIZED VIEW  IF EXISTS median_cltv_by_loan_type_{mv_suffix};

create materialized view median_cltv_by_loan_type_{mv_suffix} as
  select 
    now() as last_updated,
    case 
    ...
    percentile_cont(0.5) within group(order by loan_value_ratio::decimal) 
    as median_lv,
  ...
  where action_taken_type = 1
    ...
    and loan_value_ratio::decimal <= 125  -- threshold
