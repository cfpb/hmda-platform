create materialized view closed_end_mortgage_open_end_loc
as
	select now() last_updated, line_of_credits, count(*) agg, date_part('year', to_timestamp(action_taken_date::varchar(8), 'yyyymmdd')) || '-Q' || date_part('quarter', to_timestamp(action_taken_date::varchar(8), 'yyyymmdd')) quarter
	...
	group by quarter, line_of_credits
	union
	select now() last_updated, line_of_credits, count(*) agg, date_part('year', to_timestamp(action_taken_date::varchar(8), 'yyyymmdd')) || '-Q' || date_part('quarter', to_timestamp(action_taken_date::varchar(8), 'yyyymmdd')) quarter
