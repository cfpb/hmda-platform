create materialized view median_credit_score_by_loan_type as
	select now() last_updated, loan_type, '' conforming_loan_limit, case when loan_type = 1 then line_of_credits else 0 end loc, percentile_cont(0.5) within group(order by credit_score_applicant) median_credit_score,
	...
	group by quarter, loan_type, conforming_loan_limit, loc
	union
	select now() last_updated, loan_type, conforming_loan_limit, case when loan_type = 1 then line_of_credits else 0 end loc, percentile_cont(0.5) within group(order by credit_score_applicant) median_credit_score,
	date_part('year', to_timestamp(action_taken_date::varchar(8), 'yyyymmdd')) || '-Q' || date_part('quarter', to_timestamp(action_taken_date::varchar(8), 'yyyymmdd')) quarter
