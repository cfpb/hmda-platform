...
class FilingPathsSpec extends InstitutionHttpApiSpec with FilingPaths {
  ...
  "return a list of filings for a financial institution" in {
    getWithCfpbHeaders("/institutions/0/filings/2017") ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
    ...
    getWithCfpbHeaders(path1.toString) ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
    ...
    getWithCfpbHeaders(path2.toString) ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
      status mustBe StatusCodes.NotFound
