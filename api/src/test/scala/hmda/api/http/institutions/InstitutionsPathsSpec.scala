  ...
  "Institutions Paths" must {
    "return a list of institutions matching the 'CFPB-HMDA-Institutions' header" in {
      ...
      Get("/institutions")
        ...
        .addHeader(RawHeader("CFPB-HMDA-Institutions", "1,2")) ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
    ...
    "return an institution by id" in {
      getWithCfpbHeaders("/institutions/0") ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
        status mustBe StatusCodes.OK
