...
class SubmissionIrsPathsSpec
  ...
  "Submission Irs Paths" must {
    "return a 200" in {
      getWithCfpbHeaders("/institutions/0/filings/2017/submissions/1/irs") ~> institutionsRoutes(querySupervisor) ~> check {
    ...
    "return a CSV" in {
      getWithCfpbHeaders("/institutions/0/filings/2017/submissions/1/irs/csv") ~> institutionsRoutes(querySupervisor) ~> check {
        status mustBe StatusCodes.OK
