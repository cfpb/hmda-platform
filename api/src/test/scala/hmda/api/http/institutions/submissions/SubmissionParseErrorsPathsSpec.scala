...
class SubmissionParseErrorsPathsSpec extends InstitutionHttpApiSpec {
  ...
  "Submission Parse Errors Path" must {
    "return no errors for an unparsed submission" in {
      getWithCfpbHeaders("/institutions/0/filings/2017/submissions/1/parseErrors") ~> institutionsRoutes(querySupervisor) ~> check {
    ...
    "return first page (up to 20 errors) if request doesn't include 'page' query param" in {
      getWithCfpbHeaders("/institutions/0/filings/2017/submissions/2/parseErrors") ~> institutionsRoutes(querySupervisor) ~> check {
        status mustBe StatusCodes.OK
        val summary = responseAs[ParsingErrorSummary]
    ...
    "return next 20 errors on page 2" in {
      getWithCfpbHeaders("/institutions/0/filings/2017/submissions/2/parseErrors?page=2") ~> institutionsRoutes(querySupervisor) ~> check {
        status mustBe StatusCodes.OK
        val summary = responseAs[ParsingErrorSummary]
        summary.transmittalSheetErrors mustBe List("TS 1", "TS 2")
        summary.larErrors.size mustBe 20
    ...
    "return last 2 errors on page 3" in {
      getWithCfpbHeaders("/institutions/0/filings/2017/submissions/2/parseErrors?page=3") ~> institutionsRoutes(querySupervisor) ~> check {
        status mustBe StatusCodes.OK
        val summary = responseAs[ParsingErrorSummary]
        summary.transmittalSheetErrors mustBe List("TS 1", "TS 2")
    ...
    "Return 404 for nonexistent institution" in {
      getWithCfpbHeaders("/institutions/xxxxx/filings/2017/submissions/1/parseErrors") ~> institutionsRoutes(querySupervisor) ~> check {
      ...
      getWithCfpbHeaders("/institutions/xxxxx/filings/1980/submissions/0/parseErrors") ~> institutionsRoutes(querySupervisor) ~> check {
        status mustBe StatusCodes.NotFound
        responseAs[ErrorResponse].message mustBe "Institution xxxxx not found"
      }
    }
    "Return 404 for nonexistent filing period" in {
      getWithCfpbHeaders("/institutions/0/filings/1980/submissions/1/parseErrors") ~> institutionsRoutes(querySupervisor) ~> check {
        status mustBe StatusCodes.NotFound
    ...
    "Return 404 for nonexistent submission" in {
      getWithCfpbHeaders("/institutions/0/filings/2017/submissions/0/parseErrors") ~> institutionsRoutes(querySupervisor) ~> check {
        status mustBe StatusCodes.NotFound
