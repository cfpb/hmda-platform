...
class SubmissionBasePathsSpec extends InstitutionHttpApiSpec {
  ...
  "Submission Paths" must {
    "return not found when looking for a latest submission for non existent institution" in {
      ...
      getWithCfpbHeaders(path.toString) ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
    ...
    "create a new submission" in {
      postWithCfpbHeaders("/institutions/0/filings/2017/submissions") ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
    ...
    "find the latest submission for an institution" in {
      getWithCfpbHeaders("/institutions/0/filings/2017/submissions/latest") ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
    ...
    "fail creating a new submission for a non existent institution" in {
      ...
      postWithCfpbHeaders(path.toString) ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
        status mustBe StatusCodes.NotFound
    ...
    "return 405 when trying to POST to the /latest endpoint" in {
      postWithCfpbHeaders("/institutions/0/filings/2017/submissions/latest") ~> institutionsRoutes(supervisor, querySupervisor, validationStats) ~> check {
