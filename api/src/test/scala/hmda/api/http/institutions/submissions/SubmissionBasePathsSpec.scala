...
class SubmissionBasePathsSpec extends InstitutionHttpApiSpec {
  ...
  "Submission Paths" must {
    ...
    "create a new submission" in {
      postWithCfpbHeaders("/institutions/0/filings/2017/submissions") ~> institutionsRoutes(querySupervisor) ~> check {
    ...
    "find the latest submission for an institution" in {
      getWithCfpbHeaders("/institutions/0/filings/2017/submissions/latest") ~> institutionsRoutes(querySupervisor) ~> check {
    ...
    "fail creating a new submission for a non existent institution" in {
      ...
      postWithCfpbHeaders(path.toString) ~> institutionsRoutes(querySupervisor) ~> check {
        status mustBe StatusCodes.NotFound
    ...
    "return 405 when trying to POST to the /latest endpoint" in {
      postWithCfpbHeaders("/institutions/0/filings/2017/submissions/latest") ~> institutionsRoutes(querySupervisor) ~> check {
