...
class PublicHttpApiSpec extends WordSpec with MustMatchers with BeforeAndAfterAll
  ...
  val lar1 = larGen.sample.getOrElse(LoanApplicationRegister.empty).copy(respondentId = "0")
  val lar2 = larGen.sample.getOrElse(LoanApplicationRegister.empty).copy(respondentId = "0")
  val p = "2017"
