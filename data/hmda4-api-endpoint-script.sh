...
for i in $(seq 1 $1); do 
	...
	UPLOAD_FILE="Upload-file: $(http --check-status --ignore-stdin --form POST  https://hmda4.demo.cfpb.gov/v2/filing/institutions/B90YWS6AFX2LGWOXJ1LD/filings/2018/submissions/$sequenceNumber file@clean_test_files/bank_0/clean_file_10000_rows_Bank0_syntax_validity.txt "authorization:Bearer $token" 2>&1 | jq '.id.sequenceNumber')"
