...
object DisclosureReports {
  ...
  def main(args: Array[String]): Unit = {
    ...
    Consumer
      .committableSource(consumerSettings,
                         Subscriptions.topics("disclosure-test-1"))
      .map { msg =>
  ...
  def completeIfNoElements[A](duration: FiniteDuration,
                              completionMarker: A): Flow[A, A, NotUsed] =
  ...
  def processKafkaRecord(
    ...
    val leiDetails = spark.read
      ...
      .option(
        "dbtable",
        s"(select lei, institution_name as institutionName from transmittalsheet2018 where lei = '${msg.record
          .key()}') as transmittalsheet")
      .load()
      .as[Institution]
      .collect()
      .head
    ...
    val lookupMap: Map[(Int, Int), StateMapping] = {
      spark.read
        .option("header", "true")
        .csv("s3a://cfpb-hmda-public/dev/reports/disclosure_spark/2018/cbsa_county_name.csv")
        .select(
    ...
    val cachedRecordsDf =
      spark.read
        ...
        .option(
          "dbtable",
          s"(select * from modifiedlar2018 where lei = '${msg.record.key()}' and filing_year = 2018) as mlar")
        .load()
        .cache()
