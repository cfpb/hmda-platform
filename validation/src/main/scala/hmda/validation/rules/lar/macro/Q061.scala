...
object Q061 extends AggregateEditCheck[LoanApplicationRegisterSource, LoanApplicationRegister] with RateSpreadUtils {
  ...
  override def apply[as: AS, mat: MAT, ec: EC](lars: LoanApplicationRegisterSource): Future[Result] = {
    ...
    val firstLienPropertyLoans =
      count(lars
        ...
        .filter(lar => rateSpreadToDouble(lar.rateSpread) > 5.0))
