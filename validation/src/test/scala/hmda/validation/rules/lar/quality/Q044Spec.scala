...
class Q044Spec extends LarEditCheckSpec with BadValueUtils {
  ...
  property("Valid when ratespread less than or equal to 6.5") {
    forAll(larGen, Gen.choose(Double.MinValue, 6.5)) { (lar, x) =>
  ...
  property("Invalid when hoepa not 1 and other conditions met") {
    forAll(larGen, Gen.choose(6.5, Double.MaxValue), intOtherThan(1)) { (lar, r, h) =>
      val newLar = lar.copy(
