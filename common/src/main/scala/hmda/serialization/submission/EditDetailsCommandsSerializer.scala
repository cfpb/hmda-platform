  ...
  override def fromBinary(bytes: Array[Byte], manifest: String): AnyRef =
    manifest match {
      case PersistEditDetailsManifest =>
        persistEditDetailsFromProtobuf(
          PersistEditDetailsMessage.parseFrom(bytes),
          resolver)
      case GetEditRowCountManifest =>
        getEditRowCountFromProtobuf(GetEditRowCountMessage.parseFrom(bytes),
                                    resolver)
      case GetEditDetailsManifest =>
        getEditDetailsFromProtobuf(GetEditDetailsMessage.parseFrom(bytes),
                                   resolver)
      case _ =>
