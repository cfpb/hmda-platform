...
class DataBrowserDirectivesSpec extends WordSpec with ScalatestRouteTest with Matchers with MockFactory with ScalaFutures {
  "DataBrowserDirectives" must {
    "obtainDataSource runs a query directly against the cache if the data is already present" in {
      ...
      (fileService.retrieveDataUrl _).expects(*, *).returns(Task.now(url.some))
      (queryService.fetchData _).expects(*).returns(Source.failed(new RuntimeException("should not pull")))

      val running = obtainDataSource(fileService, queryService)(Nil, Commas).runToFuture
      scheduler.tick()
    ...
    "extractFieldsForRawQueries extracts non-mandatory query parameters and reports errors when too many criteria are provided" in {
      val route: Route = failingRoute(extractFieldsForRawQueries)
    ...
    "extractNonMandatoryQueryFields prevents you from providing valid query parameters with invalid values" in {
      Get("/?construction_methods=999") ~> failingRoute(extractNonMandatoryQueryFields) ~> check {
        response.status shouldBe StatusCodes.BadRequest
      }

      Get("/?loan_products=999") ~> failingRoute(extractNonMandatoryQueryFields) ~> check {
        response.status shouldBe StatusCodes.BadRequest
      }

      Get("/?loan_purposes=999") ~> failingRoute(extractNonMandatoryQueryFields) ~> check {
        response.status shouldBe StatusCodes.BadRequest
      }

      Get("/?loan_types=999") ~> failingRoute(extractNonMandatoryQueryFields) ~> check {
        response.status shouldBe StatusCodes.BadRequest
      }

      Get("/?sexes=machine") ~> failingRoute(extractNonMandatoryQueryFields) ~> check {
        response.status shouldBe StatusCodes.BadRequest
      }

      Get("/?total_units=wrong") ~> failingRoute(extractNonMandatoryQueryFields) ~> check {
        response.status shouldBe StatusCodes.BadRequest
      }

      Get("/?races=rainbow") ~> failingRoute(extractNonMandatoryQueryFields) ~> check {
        response.status shouldBe StatusCodes.BadRequest
      }

      Get("/?dwelling_categories=hut") ~> failingRoute(extractNonMandatoryQueryFields) ~> check {
        response.status shouldBe StatusCodes.BadRequest
      }

      Get("/?lien_statuses=999") ~> failingRoute(extractNonMandatoryQueryFields) ~> check {
        response.status shouldBe StatusCodes.BadRequest
      }

      Get("/?ethnicities=offworld") ~> Route.seal(failingRoute(extractNonMandatoryQueryFields)) ~> check {
        response.status shouldBe StatusCodes.NotFound
      }

      Get("/?actions_taken=99999") ~> Route.seal(failingRoute(extractNonMandatoryQueryFields)) ~> check {
        response.status shouldBe StatusCodes.NotFound
      }
    }

    "extractNonMandatoryQueryFields (and primitive combinators) note that all query fields are present if you provide all possible query field values" in {
      val expectedTrue: List[QueryField] => Assertion = _.forall(_.isAllSelected == true) shouldBe true

      Get(s"/?ethnicities=${Ethnicity.values.map(_.entryName).map(escape).mkString(",")}") ~> passingRoute(
        extractNonMandatoryQueryFields
      )(
        expectedTrue
      ) ~> check {
    ...
    "extractYearsAndMsaAndStateAndCountyAndLEIBrowserFields prevents you from providing too many parameters" in {
      val route: Route = failingRoute(extractYearsAndMsaAndStateAndCountyAndLEIBrowserFields)
    ...
    "extractFieldsForCount should complain when you provide no fields" in {
      Get("/") ~> failingRoute(extractFieldsForCount) ~> check {
        response.status shouldBe StatusCodes.BadRequest
    ...
    "extractFieldsForAggregation should complain if you do not provide any fields" in {
      val route = failingRoute(extractFieldsForAggregation)
  ...
  def passingRoute(inner: (List[QueryField] => Route) => Route)(expected: List[QueryField] => Assertion): Route = get {
    inner { queries =>
      expected(queries)
      complete(StatusCodes.OK)
    }
  ...
  def failingRoute(inner: (List[QueryField] => Route) => Route): Route = get {
    inner { queries =>
