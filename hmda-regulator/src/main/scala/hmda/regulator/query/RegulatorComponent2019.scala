...
import hmda.query.ts._2019.TransmittalSheetEntity
import hmda.regulator.query.lar._2019._
import hmda.regulator.query.panel._2019.{InstitutionEmailEntity2019, InstitutionEntity2019}
...
trait RegulatorComponent2019 {

  import dbConfig.profile.api._

  class InstitutionsTable2019(tag: Tag)
    extends Table[InstitutionEntity2019](tag, "institutions2019") {
    ...
    override def * =
      (lei,
        ...
        topHolderIdRssd,
        topHolderName,
        hmdaFiler) <> (InstitutionEntity2019.tupled, InstitutionEntity2019.unapply)
  }
  ...
  class InstitutionRepository2019(val config: DatabaseConfig[JdbcProfile])
    ...
    override val table: config.profile.api.TableQuery[InstitutionsTable2019] =
      institutionsTable2019

    override def getId(row: InstitutionsTable2019): config.profile.api.Rep[Id] =
      row.lei

    def createSchema() = db.run(table.schema.create)
    def dropSchema() = db.run(table.schema.drop)

    def insert(institution: InstitutionEntity2019): Future[Int] = {
      db.run(table += institution)
    }

    def findByLei(lei: String): Future[Seq[InstitutionEntity2019]] = {
      db.run(table.filter(_.lei === lei).result)
    }

    //(x => (x.isX && x.name == "xyz"))
    def findActiveFilers(
                          bankIgnoreList: Array[String]): Future[Seq[InstitutionEntity2019]] = {
      db.run(
        table
    ...
    def getAllInstitutions(): Future[Seq[InstitutionEntity2019]] = {
      db.run(table.result)
    }
  ...
  class InstitutionEmailsTable2019(tag: Tag)
    extends Table[InstitutionEmailEntity2019](tag, "institutions_emails_2019") {
    def id = column[Int]("id", O.PrimaryKey, O.AutoInc)
    def lei = column[String]("lei")
    def emailDomain = column[String]("email_domain")

    def * =
      (id, lei, emailDomain) <> (InstitutionEmailEntity2019.tupled, InstitutionEmailEntity2019.unapply)
  ...
  val institutionEmailsTable2019 = TableQuery[InstitutionEmailsTable2019]

  class InstitutionEmailsRepository2019(val config: DatabaseConfig[JdbcProfile])
    extends TableRepository[InstitutionEmailsTable2019, Int] {
    val table = institutionEmailsTable2019
    def getId(table: InstitutionEmailsTable2019) = table.id
    ...
    def getAllDomains(): Future[Seq[InstitutionEmailEntity2019]] = {
      db.run(table.result)
    }

  }
  class TransmittalSheetTable2019(tag: Tag)
  ...
  val transmittalSheetTable2019 = TableQuery[TransmittalSheetTable2019]

  class TransmittalSheetRepository2019(val config: DatabaseConfig[JdbcProfile])
    extends TableRepository[TransmittalSheetTable2019, String] {

    override val table: config.profile.api.TableQuery[TransmittalSheetTable2019] =
      transmittalSheetTable2019

    override def getId(row: TransmittalSheetTable2019): config.profile.api.Rep[Id] =
      row.lei
  ...
  class LarTable2019(tag: Tag)
    ...
    def * =
      (larPartOneProjection,
        ...
        larPartSixProjection) <> ((LarEntityImpl2019.apply _).tupled, LarEntityImpl2019.unapply)
  ...
  val larTable2019 = TableQuery[LarTable2019]

  class LarRepository2019(val config: DatabaseConfig[JdbcProfile])
    extends TableRepository[LarTable2019, String] {

    override val table: config.profile.api.TableQuery[LarTable2019] =
      larTable2019

    override def getId(row: LarTable2019): config.profile.api.Rep[Id] =
      row.lei
