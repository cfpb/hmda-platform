...
import hmda.regulator.query.lar._2018._
import hmda.regulator.query.panel._2018.{InstitutionEmailEntity2018, InstitutionEntity2018}
...
trait RegulatorComponent2018 {
  ...
  class InstitutionEmailsTable2018(tag: Tag)
      extends Table[InstitutionEmailEntity2018](tag, "institutions_emails_2018") {
    def id = column[Int]("id", O.PrimaryKey, O.AutoInc)
    def lei = column[String]("lei")
    def emailDomain = column[String]("email_domain")

    def * =
      (id, lei, emailDomain) <> (InstitutionEmailEntity2018.tupled, InstitutionEmailEntity2018.unapply)
  ...
  val institutionEmailsTable2018 = TableQuery[InstitutionEmailsTable2018]

  class InstitutionEmailsRepository2018(val config: DatabaseConfig[JdbcProfile])
      extends TableRepository[InstitutionEmailsTable2018, Int] {
    val table = institutionEmailsTable2018
    def getId(table: InstitutionEmailsTable2018) = table.id
    ...
    def getAllDomains(): Future[Seq[InstitutionEmailEntity2018]] = {
      db.run(table.result)
    }

  }
  class TransmittalSheetTable2018(tag: Tag)
  ...
  val transmittalSheetTable2018 = TableQuery[TransmittalSheetTable2018]

  class TransmittalSheetRepository2018(val config: DatabaseConfig[JdbcProfile])
      extends TableRepository[TransmittalSheetTable2018, String] {

    override val table: config.profile.api.TableQuery[TransmittalSheetTable2018] =
      transmittalSheetTable2018

    override def getId(row: TransmittalSheetTable2018): config.profile.api.Rep[Id] =
      row.lei
  ...
  val larTable2018 = TableQuery[LarTable2018]

  class LarRepository2018(val config: DatabaseConfig[JdbcProfile])
      extends TableRepository[LarTable2018, String] {

    override val table: config.profile.api.TableQuery[LarTable2018] =
      larTable2018

    override def getId(row: LarTable2018): config.profile.api.Rep[Id] =
      row.lei
