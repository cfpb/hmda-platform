...
testResults=$(sed -n '/failure/,$p' scripts/results-allclear-"${filingYear}"-"${testType}".txt)

#escape newman output for curl command
data="$( jq -nc --arg str "# $8-${filingYear}  Filing API OK! (Filing $testType): $testResults" '{"text": $str}' )"
