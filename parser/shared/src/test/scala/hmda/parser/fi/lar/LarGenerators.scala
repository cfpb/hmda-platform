...
trait LarGenerators extends FIGenerators {
  ...
  implicit def actionDateGen: Gen[Int] = dateGen.map(_.toInt)

  implicit def geographyGen: Gen[Geography] = {
    for {
      msa <- optional(stringOfN(5, Gen.numChar), "NA") // actually it's more specific; do we care, for this purpose?
      state <- optional(stringOfN(2, Gen.numChar), "NA") // TODO limit to real ones http://www2.census.gov/geo/docs/reference/state.txt
      county <- optional(stringOfN(3, Gen.numChar), "NA") // but for this one likely just allow any 3 digits?
