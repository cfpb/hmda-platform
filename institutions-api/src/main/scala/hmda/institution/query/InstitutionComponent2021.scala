...
trait InstitutionComponent2021 {

  import dbConfig.profile.api._

  class InstitutionsTable2021(tag: Tag, tableName: String) extends Table[InstitutionEntity](tag, tableName) {
  ...
  val institutionsTable2021 = TableQuery[InstitutionsTable2021]((tag: Tag) => new InstitutionsTable2021(tag, "institutions2021"))

  class InstitutionRepository2021(val config: DatabaseConfig[JdbcProfile], tableName: String)
      extends TableRepository[InstitutionsTable2021, String] {
    val institutionsTable2021               = TableQuery[InstitutionsTable2021]((tag: Tag) => new InstitutionsTable2021(tag, tableName))
    val table                               = institutionsTable2021
    def getId(table: InstitutionsTable2021) = table.lei
  ...
  class InstitutionRepository2021Beta(val config: DatabaseConfig[JdbcProfile], tableName: String)
      extends TableRepository[InstitutionsTable2021, String] {
    val institutionsTable2021Beta = TableQuery[InstitutionsTable2021]((tag: Tag) => new InstitutionsTable2021(tag, tableName))
    val table                     = institutionsTable2021Beta
